...
import { callSuggestionListDetailPersonalAPI, callUpdateInterviewSuggestionResponseAPI } from '../../apis/match/MatchAPICalls';

function SuggestionListDetailPersonal() {
  const dispatch = useDispatch();
  const interviewSuggestionCode = useParams().interviewSuggestionCode;
  const navigate = useNavigate();
  const interviewSuggestionInfo = useSelector(state => state.suggestionListDetailPersonalReducer);
    ...
    <>
      <Grid container textAlign='center'>
        <Grid item xs={12} textAlign='left'>
            <Typography fontSize='32px' paddingBottom='10px' borderBottom='1px solid black'>면접 제안서</Typography>
        ...
        <Grid item xs={10} marginBottom='20px'>
          <Typography fontSize='30px'>{interviewSuggestionInfo.comName}</Typography>
        </Grid>
        <Grid item xs={2}>
        ...
        <Grid
          ...
          xs={10}
        >
            <Typography fontSize='30px'>{interviewSuggestionInfo.interviewDate}</Typography>
          {/* <FormControl
          ...
          <Box>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (responseOffer) {
                    const result = await callUpdateInterviewSuggestionResponseAPI(interviewSuggestionCode, "Y");
                    if(result==='success'){
                        alert("승인 완료하였습니다.");
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (responseOffer) {
                    const result = await callUpdateInterviewSuggestionResponseAPI(interviewSuggestionCode, "N");
                    if(result==='success'){
                        alert("승인 완료하였습니다.");
                        navigate("/mypage/suggestion-list");
                    }else{
